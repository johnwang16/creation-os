...
# Optional: compile-time hook for an in-process BitNet adapter (scaffold).
# Runtime: COS_BITNET_NATIVE=1 requests the path (still falls back until linked).
ifeq ($(BITNET_NATIVE),1)
CFLAGS += -DCREATION_OS_BITNET_NATIVE=1
endif
...
help:
	...
	@echo "  check-mcp-stdio — tools/mcp/sigma_server.py JSON-RPC (merge-gate)"
...
# Lightweight cos CLI + σ-reinforce + neuro-symbolic bridge + demo (+ optional verify).
# For the full AGI aggregate (state ledger, episodic, c2pa, cos-serve, voice, Ω), run:
#   make check-agi
merge-gate-cos-cli: cos check-sigma check-neuro-symbolic
	./cos demo
	@set -e; PORT="$${COS_BITNET_SERVER_PORT:-11434}"; \
	if command -v curl >/dev/null 2>&1; then \
	  if curl -sfS --max-time 1 "http://127.0.0.1:$$PORT/api/tags" >/dev/null 2>&1; then \
	    echo "Paris is France." | ./cos verify >/dev/null || exit 1; \
	  else \
	    echo "merge-gate-cos-cli: skip cos verify (no Ollama on port $$PORT)"; \
	  fi; \
	else \
	  echo "merge-gate-cos-cli: skip cos verify (curl not installed)"; \
	fi
	@echo "merge-gate-cos-cli: OK"
...
# Alias: minimal σ-kernel smoke (used by merge-gate-cos-cli).
check-sigma: check-sigma-reinforce
	@echo "check-sigma: OK (alias of check-sigma-reinforce)"
...
cos-stamp: src/cli/cos_stamp.c src/sigma/c2pa_sigma.c src/sigma/semantic_sigma.c src/sigma/adaptive_sampling.c \
		src/import/bitnet_server.c src/sigma/text_similarity.c \
		$(LICENSE_KERNEL_SRCS)
	$(CC) $(CFLAGS) $(LICENSE_KERNEL_INC) -Isrc/sigma -Isrc/import -o $@ \
	    src/cli/cos_stamp.c src/sigma/c2pa_sigma.c src/sigma/semantic_sigma.c src/sigma/adaptive_sampling.c \
	    src/import/bitnet_server.c src/sigma/text_similarity.c \
	    $(LICENSE_KERNEL_SRCS) $(LDFLAGS)
...
creation_os_check_bitnet_native: tests/agi/check_bitnet_native_main.c \
		src/import/bitnet_native.c
	$(CC) $(CFLAGS) -Isrc/import -Isrc/sigma -o $@ \
	    tests/agi/check_bitnet_native_main.c src/import/bitnet_native.c \
	    $(LDFLAGS)

check-bitnet-native: creation_os_check_bitnet_native
	@./creation_os_check_bitnet_native
...
# Python stdio MCP (tools/mcp/sigma_server.py) — protocol only; no inference backend.
check-mcp-stdio:
